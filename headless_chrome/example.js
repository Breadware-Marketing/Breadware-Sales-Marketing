...
(async () => {
  ...
  for (i = 0; i < companies_test.length; i++) {
    ...
    await page
      .goto(
        "https://www.linkedin.com/sales/search?pivotType=EMPLOYEE&count=100&facet=G&facet=CC&facet.G=us%3A0&pivotId=" +
          companies_test[i].companyId,
    ...
    const leads = await page
      .evaluate(() => {
        ...
        try {
          ...
          console.log("Initial Data Could Not be Loaded");
        }
      })
    ...
    function lists(potential_leads) {
      ...
      if (leadsWithCoolPoints.length == 0) {
        console.log("none cool enough");
      } else if (leadsWithCoolPoints.length > 1) {
        // sort
      ...
      console.log(leadsWithCoolPoints)
      return leadsWithCoolPoints
    }
    results = results.concat(lists(leads))
    await page.close();
  }
  // console.log(result)
  await browser.close();
